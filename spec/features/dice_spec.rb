...
describe "/process_roll?dice=[RANDOM_DICE]&sides=[RANDOM_SIDES]" do
  it "displays the outcome of rolling a random number of dice with a random number of sides in an ordered (numbered) list", points: 1 do
...
describe "/process_roll?dice=[RANDOM_DICE]&sides=[RANDOM_SIDES]" do
  it "has a link to re-roll the same dice and sides again", points: 1 do
    ...
    expect(page).to have_link("Roll #{random_dice}d#{random_sides} again", href: "/process_roll?dice=#{random_dice}&sides=#{random_sides}")
  end
end
